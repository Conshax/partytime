    ...
    ({ feedUpdate, phaseUpdate }, { phase, tag, fn, nodeTransform, supportCheck, name }) => {
      ...
      if (tagSupported) {
        ...
        try {
          ...
          console.warn(err, `Exception thrown while trying to parse feed tag ${tagName}`);
        }
      }

      console.trace(`Feed doesn't support ${tagName}`, node, tagSupported);
      return {
        feedUpdate,
        phaseUpdate,
      };
...
export function updateItem(item: XmlNode, feed: RSSFeed, itemUpdates = items): ItemUpdateResult {
  return itemUpdates.reduce(
    ({ itemUpdate, phaseUpdate }, { phase, tag, fn, nodeTransform, supportCheck, name }) => {
      const tagName = tag;
      console.trace(`Checking feed item ${tagName} support`);
      ...
      if (tagSupported) {
        console.info(`Feed item supports ${tagName}`);
        try {
          ...
          console.warn(err, `Exception thrown while trying to parse item tag ${tagName}`);
        }
      }
      console.trace(`Feed item doesn't support ${tagName}`, node, tagSupported);
      return {
        itemUpdate,
        phaseUpdate,
      };
